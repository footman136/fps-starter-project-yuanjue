...
namespace Improbable.Gdk.Guns
{
    public class ClientShooting : MonoBehaviour, IRequiresGun
    {
        ...
        public Vector3 getShotPos(float range, Ray ray)
        {
            var hitLocation = ray.origin + ray.direction * range;
            var hitSomething = false;
            var entityId = 0L;
            ...
            if (Physics.Raycast(ray, out var hit, range, shootingLayerMask))
            {
                hitSomething = true;
                hitLocation = hit.point;
                hitLocation = hitLocation - spatial.Worker.Origin;
                var spatialEntity = hit.transform.root.GetComponent<LinkedEntityComponent>();
        ...
        public ShotInfo FireShot(float range, Ray ray,out Transform shottrans )
        {
            var hitLocation = ray.origin + ray.direction * range;
            var hitSomething = false;
            var entityId = 0L;
            ...
            if (Physics.Raycast(ray, out var hit, range, shootingLayerMask))
            {
                ...
                hitLocation = hit.point;
                var spatialEntity = hit.transform.root.GetComponent<LinkedEntityComponent>();
                var cc = hit.transform.root.GetComponent<CharacterController>();
                if ((spatialEntity != null)&&(cc!=null))
                {
                    entityId = spatialEntity.EntityId.Id;
                    shottrans = hit.transform.root;
                }
            }
            ...
            var shotInfo = new ShotInfo()
            {
                ...
                EntityIdShot = spatial.EntityId.Id
            };
